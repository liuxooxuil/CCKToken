...
// File: contracts/bbk/CCToken02.sol
...
contract CCKToken is ERC20, Ownable, IERC3643, ReentrancyGuard {
    ...
    address public Payment = 0x10a3e17511110Add0CeBcd48B93425283Db5e06D;
    ...
    function exchange(uint256 inputAmount, address targetAddress) external nonReentrant {
        ...
        // transfer(Payment, inputAmount); // 地址转型
    ...
    function propose(
        ...
        if (proposalType == ProposalType.Name) {
            ...
            require(target == address(0) && bytes(name).length == 0 && oldVoters.length == 0 && newVoters.length == 0, "");
        }
