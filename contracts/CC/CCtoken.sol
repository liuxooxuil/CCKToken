...
contract CCKToken is ERC20, Ownable, IERC3643, ReentrancyGuard {
    uint256 public TOTAL_SUPPLY = 500_000; // Adjusted to integer units (decimals = 0)
    ...
    uint256 public constant DISTRIBUTION_AMOUNT = 1; // Adjusted to integer units
    uint256 public lastAutoBatchMintTimestamp;
    uint256 public constant AUTO_BATCH_MINT_INTERVAL = 15 minutes;
    uint256 public constant BATCH_MINT_AMOUNT = 100; // Adjusted to integer units
    ...
    function deposit(address tokenAddress, address target, uint256 amount, uint8 tokenDecimals) external nonReentrant returns (bool success) {
        require(!contractPaused, "Contract paused");
        ...
        // 1:1 minting, adjusted for token decimals
        uint256 cckAmount = amount / (10 ** tokenDecimals); // Convert to CCK's integer units
        require(cckAmount > 0, "CCK amount must be greater than 0");
        require(mintedAmount + cckAmount <= TOTAL_SUPPLY, "Exceeds total supply");
